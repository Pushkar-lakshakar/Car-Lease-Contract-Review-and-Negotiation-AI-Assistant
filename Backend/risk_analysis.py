...
def calculate_contract_fairness(contract_data: Dict, vehicle_data: Dict, market_price: Optional[Dict] = None) -> Dict:
    """
    ...
    # 2. Financial
    financial_analysis = analyze_financial_terms(contract_data, vehicle_data, market_price)
    ...
    return {
        ...
        "market_price_estimation": market_price
    }


def analyze_financial_terms(contract_data: Dict, vehicle_data: Dict, market_price: Optional[Dict] = None) -> Dict:
    ...
    # 6. Check if monthly rent is reasonable compared to vehicle value (Standard Heuristic)
    msrp = None
    if vehicle_data.get("status") == "success":
        msrp = vehicle_data.get("result", {}).get("market_info", {}).get("msrp")
    
    # Overwrite MSRP with AI Estimation if available (Priority)
    if market_price and market_price.get("status") == "success":
        msrp = market_price.get("midpoint")

    if monthly_rent_str != "Not Found" and msrp:
        monthly_rent, _ = extract_currency_amount(monthly_rent_str)
        if monthly_rent:
            # 3-year total compared to value
            monthly_as_percentage = (monthly_rent * 12 * 3) / msrp * 100
            if monthly_as_percentage > 85:  # Slightly more lenient with AI estimation
                score_adj -= 15
                source = "AI Market Estimate" if (market_price and market_price.get("status") == "success") else "Vehicle MSRP"
                red_flags.append(f"HIGH LEASE PAYMENTS: 3-year total ({monthly_as_percentage:.1f}%) is disproportionate to {source} (₹{msrp:,.0f})")
            
            # Absolute monthly payment check based on car value
            # Typically monthly payment shouldn't exceed 2.5% of car value
            monthly_to_value = (monthly_rent / msrp) * 100
            if monthly_to_value > 3.0:
                score_adj -= 10
                red_flags.append(f"UNREASONABLE MONTHLY RENT: ₹{monthly_rent:,.0f} is {monthly_to_value:.1f}% of car value (should be < 2.5%)")
...
def quick_risk_assessment(contract_data: Dict, vehicle_data: Dict, market_price: Optional[Dict] = None) -> Dict:
    """Simple wrapper for API"""
    return calculate_contract_fairness(contract_data, vehicle_data, market_price)
